...
Now we are going to use State method for making it more modular
...
Let' do some changes inside `Display.jsx`
```JSX
import styles from "./Display.module.css";
...
Now we want to make the display `read only` How can we do that ?

so here we need to some changes inside `Display.jsx` file
```JSX
import styles from "./Display.module.css";
...
Now let's just check whether the value we are defining in `App.jsx` is displaying properly or not
```JSX
import React, { useState } from "react";
import styles from "./App.module.css";
import Display from "./components/Display";
...
See it is rendering properly it means everything is `OK` 
![[Pasted image 20241026123108.png]]




Now we are going to add the `ButtonClick` functionality so that if we click any button it will say that **Button is clicked**

Let's do some changes inside `App.jsx` file to reflect changes
```JSX
import React, { useState } from "react";
import styles from "./App.module.css";
import Display from "./components/Display";
...
    <div className={styles.calculator}>
      <Display displayVal={calVal} />
      <ButtonsContainer onButtonClick={() => console.log("Button is clicked")} />
    </div>
  );
}
...
Let's do some changes inside `ButtonsContainer.jsx` to implement the functionality
```JSX
import styles from "./ButtonsContainer.module.css";
...
    <div className={styles.buttonContainer}>
      {buttonNames.map((buttonName) => (
        <button className={styles.button} onClick={onButtonClick} >{buttonName}</button>
      ))}
    </div>
  );
};
...
Now we can also write this way also for more suitable and modularity inside `App.jsx` i.e. we can also define the `onButtonClick` this way also for better understanding
...
![[Pasted image 20241026125802.png]]



Now we can pass and print `event` also inside `App.jsx` i.e. on button click event should get printed
```JSX
import React, { useState } from "react";
import styles from "./App.module.css";
import Display from "./components/Display";
...
![[Pasted image 20241026130000.png]]



Now we want to know which button is being clicked so that we can print that button in the console

so let's do some changes inside `App.jsx`
```JSX
import React, { useState } from "react";
import styles from "./App.module.css";
import Display from "./components/Display";
...
see the changes inside console 
![[Pasted image 20241026131346.png]]



We we need to add the functionality of the buttons excluding i.e. C =  clear and = = Equals and we want to show the display the entered values in the display of the calculator instead in console
...
![[Pasted image 20241026153241.png]]



We we need to add the functionality of the buttons  i.e. C =  clear

```JSX
setCalVal("");
```

implement the above code inside the `App.jsx`
...
We we need to add the functionality of the buttons i.e.  = = Equals 
`const result = eval(calVal);` implement inside our code inside `App.jsx` 
...
![[Pasted image 20241026154345.png]]


![[Pasted image 20241026154357.png]]

![[Pasted image 20241026154416.png]]

Now you can see we have implemented all the buttons functionality of the calculator now it is working fine you can se it
